  ...
  return (
    <>
      <button
        ...
        onClick={() => select(null)}
        className={`fixed inset-0 z-20 bg-[rgba(6,12,18,0.32)] transition-opacity duration-300 sm:hidden ${
          isOpen ? "pointer-events-auto opacity-100" : "pointer-events-none opacity-0"
        }`}
      />
      <aside
        aria-hidden={!isOpen}
        className={`pointer-events-none fixed inset-x-0 bottom-0 top-auto z-30 flex items-end justify-center transition-opacity duration-300 sm:inset-y-0 sm:left-auto sm:right-0 sm:items-start sm:justify-end sm:p-4 ${
          isOpen ? "opacity-100" : "opacity-0"
        }`}
      >
        <div
          className={`flex max-h-[min(72dvh,42rem)] w-full flex-col overflow-hidden rounded-t-[30px] border border-white/10 bg-[rgba(28,25,26,0.92)] shadow-[0_26px_80px_rgba(4,16,28,0.34)] backdrop-blur-xl transition-transform duration-300 ease-out sm:h-[calc(100vh-120px)] sm:max-h-none sm:w-[378px] sm:rounded-[32px] ${
            isOpen
              ? "pointer-events-auto translate-y-0 sm:translate-x-0"
              : "pointer-events-none translate-y-[calc(100%+1.5rem)] sm:translate-x-[110%] sm:translate-y-0"
          }`}
        >
          ...
          {selectedParcel && (
            <>
              ...
              {selectedParcel.contactUrl && (
                <div className="border-t border-white/8 px-6 pb-[calc(env(safe-area-inset-bottom)+1.25rem)] pt-5">
                  <a
